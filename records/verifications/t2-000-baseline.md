...
**Official train time: 525.4s (8m 45s)** · runs passed: 3/3 · EM spread 77.4–77.6%
(all clear the 75.5% target by ≥1.9pt). Run 3's *time* is dropped as infrastructure
noise — see below; its accuracy still counts.

## Run 3 dropped from the official time (infrastructure noise)
...
minutes in pre-training I/O (model/dataset load from the shared cache volume): platform
storage variance, not anything in the submission.

**Updated 2026-07-21.** The project adopted an infrastructure-outlier rule: a run whose
wall-clock exceeds 1.5× the batch median while its accuracy is normal is a Modal I/O
stall, not algorithmic time — its time is dropped from the official mean and the run is
re-run (see [JUDGING.md](../../JUDGING.md) and `spec-t2.yaml`
`verification.infra_outlier_rule`). Run 3 meets that test exactly (953.4s > 1.5 × 527.2s =
790.8s; EM 77.38% is normal), so its **time** is dropped while its accuracy still counts.
The official time becomes the mean of the two clean runs (523.5, 527.2) = **525.4s
(8m 45s)**, down from the 668.0s (11m 08s) first recorded under the pre-rule
mean-of-all-runs. Run 3 is flagged for a re-run to restore a clean 3-seed timing set. The
raw measurements in the table above are unchanged.
...
first-20k subset — are open lanes. **The Track 2 record to beat is 8m 45s.**
