...
def next_fit_pack_baseline_membership(
    examples: Sequence[tuple[Sequence[int], Sequence[int]]],
    *,
    capacity: int,
    pad_token_id: int,
    ...
    """Return the exact historical next-fit flushes, dropping its final tail."""
    ...
    label_blocks: list[tuple[int, ...]] = []
    bin_source_indices: list[tuple[int, ...]] = []
    for bin_examples in baseline_bins:
        input_ids = tuple(
            token for example in bin_examples for token in example.input_ids
        )
        labels = tuple(
            token for example in bin_examples for token in example.labels
        )
        if len(input_ids) != len(labels) or len(input_ids) > capacity:
            raise AssertionError("next-fit produced an invalid bin")
        padding = capacity - len(input_ids)
        input_blocks.append(input_ids + (pad_token_id,) * padding)
        label_blocks.append(labels + (ignore_index,) * padding)
        bin_source_indices.append(
            tuple(example.source_index for example in bin_examples)
        )

    included = tuple(
        example for bin_examples in baseline_bins for example in bin_examples
    )
    included_indices = tuple(example.source_index for example in included)
    packed_indices = tuple(index for block in bin_source_indices for index in block)
    if packed_indices != included_indices:
        raise AssertionError("next-fit changed source-example order")
