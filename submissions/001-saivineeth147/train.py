"""Record #1 attempt: sequence packing + completion-only loss masking.

The baseline pads every ~190-token GSM8K example out to 768 (so most compute is
spent on padding) and computes loss over the question tokens it never needs to
generate. This submission fixes both:

  1. Completion-only masking — loss is computed ONLY on the answer tokens
     (the prompt is masked to -100), so every gradient is signal.
  2. Greedy sequence packing — examples are concatenated into dense 1024-token
     blocks with ~no padding, so ~5x more real tokens flow per unit of compute.

Both together let 2 epochs clear the target in a fraction of the baseline's time.
